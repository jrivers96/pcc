...
#ifndef PEARSONR_HPP_
#define PEARSONR_HPP_
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>
...
#define PR_MIC_REUSE alloc_if(0) free_if(0)
#define PR_MIC_ALLOC alloc_if(1) free_if(0)
#define PR_MIC_FREE  length(0) alloc_if(0) free_if(1)
#define PR_MIC_ALLOC_FREE alloc_if(1) free_if(1)
#endif	/*with phi*/

/*tile size*/
#define PR_PHI_TILE_DIM	4		/*must be divided by 236*/
#define PR_PHI_TILE_SIZE	(PR_PHI_TILE_DIM * PR_PHI_TILE_DIM)
#define MT_TILE_DIM		8
#define MPI_TILE_DIM	8
#define MPI_TILE_SIZE	(MPI_TILE_DIM * MPI_TILE_DIM)

/*maximum dimension size*/
#ifndef PR_MAX_MAT_DIM
#define PR_MAX_MAT_DIM			(1<<25)
#endif

/*maximum Xeon Phi buffer size*/
#define PR_PHI_BUFFER_SIZE (1 << 29)
...
class PearsonR {
public:
	PearsonR(int numVectors, int vectorSize, int numCPUThreads,
...
template<typename FloatType>
PearsonR<FloatType>::PearsonR(int numVectors, int vectorSize, int numCPUThreads,
		int numMICThreads, int micIndex, int rank, int numProcs) {
	...
	if (_numVectors > PR_MAX_MAT_DIM) {
		fprintf(stderr,
				"The number of vectors exceeds the maximum limit (%ld)\n",
				(ssize_t) PR_MAX_MAT_DIM);
		exit(-1);
	}
...
template<typename FloatType>
void PearsonR<FloatType>::generateRandomData(const int seed) {
	srand48(11);
...
template<typename FloatType>
void PearsonR<FloatType>::runSingleThreaded() {
	double stime, etime;
#ifdef VERBOSE
...
#pragma simd reduction(+:prod)
      ...
      _pearsonCorr[(ssize_t)col] = prod;
      //riversj _pearsonCorr[(ssize_t)row * _numVectors + col] = prod;
			//riversj _pearsonCorr[(ssize_t)col * _numVectors + row] = prod;
		}
	}
  fprintf(stderr, "riversj: %f \n",riversj);
...
template<typename FloatType>
void PearsonR<FloatType>::runMultiThreaded() {
	double stime, etime;
	const int tileDim = (_numVectors + MT_TILE_DIM - 1) / MT_TILE_DIM;
...
#pragma omp barrier
		...
		for (int tileRow = loRowRange; tileRow <= hiRowRange; tileRow++) {
			...
			rowStart = tileRow * MT_TILE_DIM;
			rowEnd = min(_numVectors, rowStart + MT_TILE_DIM);
			for (int tileCol = startColPerRow; tileCol <= endColPerRow;
					++tileCol) {
				colStart = tileCol * MT_TILE_DIM;
				colEnd = min(_numVectors, colStart + MT_TILE_DIM);
				/*compute the tile*/
...
#pragma simd reduction(+:prod)
            ...
            _pearsonCorr[(ssize_t) col] = prod;
            //_pearsonCorr[(ssize_t)row * _numVectors + col] = prod;
						//_pearsonCorr[(ssize_t)col * _numVectors + row] = prod;
					}
				}
			}
		}
...
#ifdef WITH_PHI
template<typename FloatType>
void PearsonR<FloatType>::runSingleXeonPhi() {
	const int tileDim = (_numVectors + PR_PHI_TILE_DIM - 1) / PR_PHI_TILE_DIM;
	const ssize_t numTiles = (ssize_t)(tileDim + 1) * tileDim / 2;
...
#pragma offload target(mic:_micIndex) inout(_numMICThreads)
	{
		...
		if (_numMICThreads < PR_PHI_TILE_DIM) {
			_numMICThreads = omp_get_num_procs();
		}
		_numMICThreads = _numMICThreads / PR_PHI_TILE_DIM * PR_PHI_TILE_DIM;
		omp_set_num_threads (_numMICThreads);
	}
	fprintf(stderr, "number of threads: %d\n", _numMICThreads);

	/*allocate intermediate memory*/
	ssize_t maxPrValuesSize = PR_PHI_BUFFER_SIZE / sizeof(FloatType); /*a total of 512 MB*/
	ssize_t alignment = _numMICThreads * PR_PHI_TILE_SIZE;
...
#pragma offload_transfer target(mic: _micIndex) \
		in(vectors: length(_numVectors * _vectorSizeAligned) PR_MIC_ALLOC) \
		nocopy(inValues: length(maxPrValuesSize) PR_MIC_ALLOC) \
		nocopy(outValues: length(maxPrValuesSize) PR_MIC_ALLOC)
	...
	/*compute mean and variance*/
	double t1 = getSysTime();
#pragma offload target(mic:_micIndex) nocopy(vectors: PR_MIC_REUSE)
	{
#pragma omp parallel
		{
			int row, col;
...
#pragma simd
	...
	/*the first round*/
	maxNumTilesPerPass = maxPrValuesSize / PR_PHI_TILE_SIZE;
	indexRangeStart = 0;
	indexRangeClose = min(numTiles, maxNumTilesPerPass);
#pragma offload target(mic:_micIndex) \
		nocopy(vectors: PR_MIC_REUSE)	\
		nocopy(inValues: PR_MIC_REUSE)	\
		in(indexRangeStart) \
		in(indexRangeClose) \
		signal(&signalVar)
	{
...
#pragma offload target(mic:_micIndex) \
		...
		numElems = (indexRangeClosePrev - indexRangeStartPrev) * PR_PHI_TILE_SIZE;
		if(numElems > maxPrValuesSize){
			fprintf(stderr, "Error: %ld > %ld\n", numElems, maxPrValuesSize);
		}
#pragma offload_transfer target(mic:_micIndex) out(outValues[0:numElems]:into(hostPrValues[0:numElems]) PR_MIC_REUSE)

    /*save the data to the correlation matrix*/
		tileOffset = 0;
    for(ssize_t idx = indexRangeStartPrev; idx < indexRangeClosePrev; ++idx, tileOffset += PR_PHI_TILE_SIZE){
      /*get the row and column index of the tile*/
      getTileCoordinate(idx, tileDim, tileRowStart, tileColStart);

      /*compute the row range*/
      tileRowStart *= PR_PHI_TILE_DIM;
      tileColStart *= PR_PHI_TILE_DIM;
      tileRowEnd = min(_numVectors, tileRowStart + PR_PHI_TILE_DIM);
      tileColEnd = min(_numVectors, tileColStart + PR_PHI_TILE_DIM);

      /*compute each tile*/
			ssize_t rowOffset = tileOffset;
      for(int row = tileRowStart; row < tileRowEnd; ++row, rowOffset += PR_PHI_TILE_DIM){
        for(int col = tileColStart, colOffset = rowOffset; col < tileColEnd; ++col, ++colOffset){
					if(row <= col){
            //riversj _pearsonCorr[(ssize_t)row * _numVectors + col] = hostPrValues[colOffset];
            //riversj _pearsonCorr[(ssize_t)col * _numVectors + row] = hostPrValues[colOffset];
          }
        }
      }
    }
...
#pragma offload_transfer target(mic: _micIndex) \
		nocopy(vectors : PR_MIC_FREE) \
		nocopy(inValues: PR_MIC_FREE) \
		nocopy(outValues: PR_MIC_FREE)
...
template<typename FloatType>
__attribute__((target(mic))) void PearsonR<FloatType>::_runSingleXeonPhiCore(
...
#ifdef SOFT_BARRIER
	/*soft barrier*/
	_softBarrierInit(_numMICThreads / PR_PHI_TILE_DIM, PR_PHI_TILE_DIM);
#endif	/*soft barrier*/

#pragma omp parallel
	{
		const int numGroups = omp_get_num_threads() / PR_PHI_TILE_DIM;
		const int tid = omp_get_thread_num() % PR_PHI_TILE_DIM;
		const int gid = omp_get_thread_num() / PR_PHI_TILE_DIM;
		...
		for (idx = indexRangeStart + gid; idx < indexRangeClose; idx += numGroups) {
			...
			/*compute row range*/
			tileRowStart *= PR_PHI_TILE_DIM;
			tileRowEnd = tileRowStart + PR_PHI_TILE_DIM;
			if(tileRowEnd > _numVectors) {
				tileRowEnd = _numVectors;
			}
			/*each work thread computes its own column index*/
			offset = (idx - indexRangeStart) * PR_PHI_TILE_SIZE + tid;
			col = tileColStart * PR_PHI_TILE_DIM + tid;
			if(col < _numVectors) {
				vecY = vectors + col * _vectorSizeAligned;
				vecX = vectors + tileRowStart * _vectorSizeAligned;
				for(row = tileRowStart; row < tileRowEnd; ++row, vecX += _vectorSizeAligned, offset += PR_PHI_TILE_DIM) {
					if(row > col) {
						continue;
					}
...
#ifdef SOFT_BARRIER
			_softBarrierSync(gid, PR_PHI_TILE_DIM, localSense);
#endif	/*soft barrier*/
		}
	}
...
template<>
__attribute__((target(mic))) void PearsonR<float>::_runSingleXeonPhiCore(
...
template<>
__attribute__((target(mic))) void PearsonR<double>::_runSingleXeonPhiCore(
...
#ifdef WITH_MPI
template<typename FloatType>
void PearsonR<FloatType>::runMPICPU() {
	double stime, etime;
	const int tileDim = (_numVectors + MPI_TILE_DIM -1) / MPI_TILE_DIM;
	const ssize_t numTiles = (ssize_t)(tileDim + 1) * tileDim / 2;
...
/*riversj
	_pearsonCorr = (FloatType*)mm_malloc( chunkSize * MPI_TILE_DIM * MPI_TILE_DIM * sizeof(FloatType), 64);
	if(!_pearsonCorr) {
		fprintf(stderr, "Memory allocation failed\n");
		exit(-1);
	}
...
#pragma simd
	...
	/*global offset of tiles*/
	offset = 0;
	for (int tileRow = loRowRange; tileRow <= hiRowRange; tileRow++) {
		rowStart = tileRow * MPI_TILE_DIM;
		rowEnd = min(_numVectors, rowStart + MPI_TILE_DIM);
		startColPerRow = (tileRow == loRowRange) ? loColRange : tileRow;
		endColPerRow = (tileRow == hiRowRange) ? hiColRange : tileDim - 1;
		for (int tileCol = startColPerRow; tileCol <= endColPerRow;
				++tileCol) {
			colStart = tileCol * MPI_TILE_DIM;
			colEnd = min(_numVectors, colStart + MPI_TILE_DIM);

			/*compute the tile*/
			vecX = _vectors + rowStart * _vectorSizeAligned;
			for (row = rowStart, rowOffset = offset; row < rowEnd; row++, vecX +=
					_vectorSizeAligned, rowOffset += MPI_TILE_DIM) {
				vecY = _vectors + colStart * _vectorSizeAligned;
...
#pragma simd reduction(+:prod)
					...
					//riversj _pearsonCorr[colOffset] = prod;
				}
			}
			/*move to the next tile*/
			offset += MPI_TILE_SIZE;
		}
	}
...
#ifdef WITH_PHI
template<typename FloatType>
void PearsonR<FloatType>::runMPIXeonPhi() {
	const int tileDim = (_numVectors + PR_PHI_TILE_DIM - 1) / PR_PHI_TILE_DIM;
	const ssize_t numTiles = (ssize_t)(tileDim + 1) * tileDim / 2;
...
#pragma offload target(mic:_micIndex) inout(_numMICThreads)
	...
	_pearsonCorr = (FloatType*)mm_malloc( chunkSize * PR_PHI_TILE_SIZE * sizeof(FloatType), 64);
	if(!_pearsonCorr) {
		fprintf(stderr, "Memory allocation failed\n");
		exit(-1);
	}
...
#pragma offload target(mic:_micIndex) \
	...
	signal(&signalVar)
		...
		numElems = (indexRangeClosePrev - indexRangeStartPrev) * PR_PHI_TILE_SIZE;
#pragma offload_transfer target(mic:_micIndex) out(outValues[0:numElems]:into(_pearsonCorr[offset:numElems]) PR_MIC_REUSE)
		offset += numElems;
	}
...
template <typename FloatType>
void PearsonR<FloatType>::transpose()
{
