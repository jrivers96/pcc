...
static void printUsage() {
	fprintf(stderr,
			"LightPCC distance [options] -m exe_mode\n");
...
int lightDistanceCorr(int argc, char* argv[]) {
	...
	if (!option._useDouble) {
		DistanceCorr<float> pr(option._numVectors, option._vectorSize,
		...
		DistanceCorr<double> pr(option._numVectors, option._vectorSize,
				option._numCPUThreads, option._numMICThreads, option._micIndex,
				option._rank, option._numProcs);
		if (option._input.length()) {
